...
describe("runPipeline — dryRun", () => {
  test("estimation is present but execution is absent", async () => {
    const result = await runPipeline(SUPPORTED_QUERY, successFetch(), { dryRun: true })
  ...
  test("fetchFn is never called during dryRun", async () => {
    const mockFetch = jest.fn()
    await runPipeline(SUPPORTED_QUERY, mockFetch as unknown as FetchFn, { dryRun: true })
    expect(mockFetch).not.toHaveBeenCalled()
  })
  ...
  test("estimation has correct step count", async () => {
    ...
    expect(result.estimation!.stepCosts.length).toBe(result.estimation!.steps.length)
  })
})
...
describe("runPipeline — successful execution", () => {
  ...
  test("execution.results length equals step count", async () => {
    const result = await runPipeline(SUPPORTED_QUERY, successFetch())
    expect(result.execution!.results.length).toBe(result.classification.steps.length)
  })
})
...
describe("runPipeline — per-step timeout", () => {
  test("step times out and errorMessage reflects it", async () => {
    ...
    expect(result.execution).toBeDefined()
    expect(result.execution!.success).toBe(false)
    expect(result.execution!.errorMessage?.toLowerCase()).toContain("timed out")
  }, 3_000)
  ...
  test("uses injected registry for URL resolution", async () => {
    ...
    const registry = new ServiceRegistry({
      Firecrawl: {
        ...
        buildQuery: (raw, ctx) => ({ url: ctx.urls[0] ?? null, mode: "scrape", rawQuery: raw }),
      },
    })
    ...
    await runPipeline(SUPPORTED_QUERY, fetchFn, { registry, continueOnUnhealthy: true })
...
describe("PipelineResult shape invariants", () => {
  ...
  test("dryRun field matches the option passed", async () => {
    const r1 = await runPipeline(SUPPORTED_QUERY, successFetch(), { dryRun: true })
    const r2 = await runPipeline(SUPPORTED_QUERY, successFetch(), { dryRun: false })
